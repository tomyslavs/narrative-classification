...
def compute_comment_article_similarity_over_time(articles, comments, emb_model,
    ...
    article_embs = emb_model.encode(article_texts, batch_size=32*2, show_progress_bar=True)
    ...
    comment_embs = emb_model.encode(comment_texts, batch_size=128*2, show_progress_bar=True)
    ...
    merged[cols_to_save].to_csv(out_all_csv, index=False)
...
def compute_emotion_shift_narrative_injection(articles, comments,
    ...
    article_engagement = (
        ...
        .agg(
            article_total_reactions=("total_reactions", "sum"),
    ...
    article_out["article_total_reactions"] = article_out["article_total_reactions"].fillna(0).astype(int)
    ...
    comments[sent_cols].to_csv(out_sent_csv, index=False)
    ...
    article_reactions = (
        ...
        .sum()
        .reset_index()
        .rename(columns={"total_reactions": "article_total_reactions"})
    )
...
# -----------------------------
# 4) COMMENTS NARRATIVE CLUSTER VISUALIZATION (UMAP / t-SNE)
# -----------------------------
...
def build_narrative_clusters(articles, emb_model, max_articles=None):
    ...
    arts = articles.copy()
    arts["article_text"] = (
        arts.get("comment", "").fillna("").astype(str)
    )

    if max_articles is not None and len(arts) > max_articles:
        arts = arts.sample(max_articles, random_state=42)

    print(f"Encoding {len(arts)} comments for clustering...")
    texts = arts["article_text"].tolist()
    embs = emb_model.encode(texts, batch_size=32*2, show_progress_bar=True)
    ...
    df_umap = arts[["portal", "article_id", "comment_id", "created_at"]].copy()
    df_umap["x"] = umap_coords[:, 0]
    df_umap["y"] = umap_coords[:, 1]

    out_csv_u = RESULTS_DIR / "comments_narrative_clusters_umap.csv"
    df_umap.to_csv(out_csv_u, index=False)
    print(f"Saved UMAP coordinates to {out_csv_u}")

    plt.figure(figsize=(9, 7))
    sns.scatterplot(data=df_umap, x="x", y="y", hue="portal", s=15, alpha=0.7)
    plt.title("Comments Narrative Cluster Visualization (UMAP) - by Portal")
    plt.xlabel("UMAP-1")
    plt.ylabel("UMAP-2")
    plt.legend(markerscale=1.5, fontsize=8)
    plt.tight_layout()
    out_png_u = RESULTS_DIR / "comments_narrative_clusters_umap_by_portal.png"
    plt.savefig(out_png_u, dpi=300)
    plt.close()
    print(f"Saved UMAP plot (by portal) to {out_png_u}")
    ...
    df_tsne = arts[["portal", "article_id", "comment_id", "created_at"]].copy()
    df_tsne["x"] = tsne_coords[:, 0]
    df_tsne["y"] = tsne_coords[:, 1]

    out_csv_t = RESULTS_DIR / "comments_narrative_clusters_tsne.csv"
    df_tsne.to_csv(out_csv_t, index=False)
    print(f"Saved t-SNE coordinates to {out_csv_t}")

    plt.figure(figsize=(9, 7))
    sns.scatterplot(data=df_tsne, x="x", y="y", hue="portal", s=15, alpha=0.7)
    plt.title("Comments Narrative Cluster Visualization (t-SNE) - by Portal")
    plt.xlabel("t-SNE-1")
    plt.ylabel("t-SNE-2")
    plt.legend(markerscale=1.5, fontsize=8)
    plt.tight_layout()
    out_png_t = RESULTS_DIR / "comments_narrative_clusters_tsne_by_portal.png"
    plt.savefig(out_png_t, dpi=300)
    plt.close()
    print(f"Saved t-SNE plot (by portal) to {out_png_t}")
